...
class PromoCodeBase(BaseModel):
    ...
    percent_basis_points: Optional[int] = Field(
        None, ge=1, le=10000, description="Percentage discount in basis points (1% = 100 basis points)"
    )
    max_discount_cents: Optional[int] = Field(
        None, ge=0, description="Maximum discount amount in cents for percentage-based promo codes"
    )
    ...
    starts_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Start date and time for the promo code validity"
    )
    ...
    def validate_discount_type(self):
        ...
        has_percent = self.percent_basis_points is not None and self.percent_basis_points > 0
...
class PromoCodeResponse(PromoCodeBase):
    ...
    usage_count: int = Field(..., ge=0, description="Number of times the promo code has been used")
    created_at: datetime = Field(..., description="Timestamp when the promo code was created")
    updated_at: datetime = Field(..., description="Timestamp when the promo code was last updated")
...
class PromoCodeUpdate(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    promo_type: Optional[PromoTypeEnum] = Field(None, description="Type of the promo code")
    value_cents: Optional[int] = Field(
        None, ge=0, description="Fixed amount discount in cents"
    )
    percent_basis_points: Optional[int] = Field(
